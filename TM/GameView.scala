...
object GameView {
  ...
  /**
	...
	def update {
		...
		for(i <- (0 until GameEngineconcreta.height)) {
		  for(j <- (0 until GameEngineconcreta.width)) {
		    print(if (GameEngineconcreta.isCellAlive(i, j))  ALIVE_CELL else DEAD_CELL);
		  }
		  println("   " + i)
		  printLine
		}
  ...
  private def makeCellAlive {
	  ...
	  do {
      print("\n Inform the row number (0 - " + (GameEngineconcreta.height - 1) + "): ")
      i = readInt
      
      print("\n Inform the column number (0 - " + (GameEngineconcreta.width - 1) + "): ")
      j = readInt
  ...
  private def validPosition(i: Int, j: Int): Boolean = {
		println(i);
		println(j);
		i >= 0 && i < GameEngineconcreta.height && j >= 0 && j < GameEngineconcreta.width
	}
